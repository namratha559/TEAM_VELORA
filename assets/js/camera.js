...
async function ensurePoseModel(){
  ...
  try{
    CAM.landmarker = await vis.PoseLandmarker.createFromOptions(fileset, {baseOptions:{...baseOptions, delegate:'GPU'}, runningMode:'VIDEO', numPoses:1});
  }catch(e){
    // GPU delegate unavailable/unsupported on this device — fall back to CPU/WASM.
    CAM.landmarker = await vis.PoseLandmarker.createFromOptions(fileset, {baseOptions:{...baseOptions, delegate:'CPU'}, runningMode:'VIDEO', numPoses:1});
  }
...
function cameraErrorMessage(e){
  const map={
    ...
    PermissionDeniedError:'Camera permission was denied. Please allow camera access in your browser settings and try again.',
    NotFoundError:'No camera was found on this device.',
    NotReadableError:'The camera is already in use by another application. Please close it and try again.',
    OverconstrainedError:'The camera does not support the requested settings.',
    SecurityError:'Camera access requires HTTPS or localhost. Please open this app using HTTPS.'
  };
  return (e && map[e.name]) || t('cam_unavailable');
}
async function startCameraTracking(w){
  const box=document.getElementById('cam-box');
  if(!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){
    box.innerHTML = `<div class="cam-warn">Camera access requires HTTPS or localhost. Please open this app using HTTPS.</div>`; return;
  }
  box.innerHTML = `<p style="font-size:12.5px;color:var(--muted);">Requesting camera permission…</p>`;
  try{
    CAM.stream = await navigator.mediaDevices.getUserMedia({video:{facingMode:'user'}, audio:false});
  }catch(e){
    box.innerHTML = `<div class="cam-warn">${cameraErrorMessage(e)}</div>
      <button class="btn btn-outline btn-sm" style="margin-top:8px;" id="camera-retry">Retry</button>`;
    const retryBtn=document.getElementById('camera-retry'); if(retryBtn) retryBtn.onclick=()=>startCameraTracking(w);
    return;
  }
  // Show the live preview immediately — do not wait for the pose model before the user can see video.
  box.innerHTML = `<div class="cam-wrap"><video id="cam-video" autoplay playsinline muted></video><canvas id="cam-overlay"></canvas></div>
    <p id="cam-model-status" style="font-size:12px;color:var(--muted);margin-top:6px;">Loading pose model…</p>
    <div class="cam-live">
  ...
  CAM.canvas.width = CAM.video.videoWidth || 640; CAM.canvas.height = CAM.video.videoHeight || 480;
  try{ await CAM.video.play(); }catch(e){ /* autoplay may already be active */ }
  ...
  try{
    ...
    const st=document.getElementById('cam-model-status'); if(st) st.textContent='';
    poseLoop(w);
  }catch(e){
    const st=document.getElementById('cam-model-status');
    if(st){ st.textContent='Pose analysis unavailable — camera preview only. You can still Cancel or Retake; angle/ROM require pose detection.'; st.style.color='var(--high)'; }
  }
}
function waitForVideoReady(video){
  return new Promise(res=>{
    if(video.readyState>=1 && video.videoWidth>0){ res(); return; } // HAVE_METADATA already available
    let done=false;
    const finish=()=>{ if(done) return; done=true; res(); };
    video.onloadedmetadata=finish;
    video.onloadeddata=finish;
    setTimeout(finish, 3000); // safety timeout in case neither event fires
  });
}
function poseLoop(w){
