...
function renderQrIfNeeded(){
  ...
  } else if(!holder.dataset.qrRetry){
    // Library script may still be mid-load (e.g. CDN fallback in flight) — retry briefly before giving up.
    holder.dataset.qrRetry='1';
    setTimeout(renderQrIfNeeded, 300);
  } else {
    holder.innerHTML = `<p style="font-size:12.5px;color:var(--high);">QR library failed to load. Patient ID: <b>${pid}</b></p>`;
  }
}
